...
from tianshou.policy import A2CPolicy_SPiAM
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--sigma_lr', default=5, type=float, help='sigma multiplier updating')
    parser.add_argument('--eps', default=1e-8, type=float, help='eps in SPiAM')
    parser.add_argument(
        "--device",
        type=str,
...
def test_a2c(args: argparse.Namespace = get_args()) -> None:
    ...
    dummy_model = [torch.zeros(1, requires_grad=True)]  # Dummy parameter
    dummy_optim = torch.optim.Adam(dummy_model, lr=args.lr)  # Use any optimizer, as it won't update parameters
    ...
    if args.lr_decay:
        ...
        lr_scheduler = LambdaLR(dummy_optim, lr_lambda=lambda epoch: 1 - epoch / max_update_num)

    def dist(loc_scale: tuple[torch.Tensor, torch.Tensor]) -> Distribution:
        loc, scale = loc_scale
        return Independent(Normal(loc, scale), 1)

    policy: A2CPolicy = A2CPolicy_SPiAM(
        actor=actor,
        critic=critic,
        optim=dummy_optim,
        dist_fn=dist,
        ...
        eps = args.eps, ### check whether it needs to change to 1e-5
    )
