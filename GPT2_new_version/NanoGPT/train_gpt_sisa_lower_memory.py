...
class ADMMParamScheduler:
    def __init__(self, optimizer, ref_optimizer, args, use_residual_adapt=False,
                 mu=10.0, tau=2.0):
        """
...
val_loss_record = []
save_path_val = "/workspace0/ow120/DDAM/modded-nanogpt/experiment_results/sisa_submit/val_loss_record_lower_memory.txt"
train_time_record = []
save_path_time = "/workspace0/ow120/DDAM/modded-nanogpt/experiment_results/sisa_submit/train_time_record_lower_memory.txt"
...
sched_sigma_rho_1 = SigmaRhoScheduler(
    dist_opt   = optimizer2_1,
    adam_opt   = optimizer1,
    base_sigma = args_1.sigma_lr,
    base_rho   = args_1.rho_lr
)
...
sched_sigma_rho = ADMMParamScheduler(optimizer2_2, optimizer1, args)
# begin logging
if master_process:
...
try:
    ...
    for step in range(args.num_iterations + 1):
        ...
        if (last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0)):
            ...
            if master_process:
                ...
                print('The sigma_lr is:', optimizer2_2.param_groups[0]['sigma_lr'])
        ...
        #param_check = list(raw_model.transformer.h.parameters())
        #print('The admm update parameter is:', param_check[0][0:8])
        #print('#############################################')
        for i in range(1, train_accumulation_steps+1):
            
            # forward pass
            optimizer2_2.set_local_batch_size(x.size(0))
            #optimizer2_1.set_local_batch_size(x.size(0))
            '''with ctx:
        ...
        for p in model.parameters():
            ...
            with model.no_sync():
                ...
                loss.backward()
                #if step == 0:
                    #normalized_factor = torch.sqrt(sum(torch.sum(p.grad ** 2) for p in raw_model.transformer.h.parameters() if p.grad is not None))

                
                #optimizer2.step(normalized_factor = normalized_factor)
                #optimizer2_1.step()
                optimizer2_2.step()
                #grads  = [p.grad for p in raw_model.transformer.h.parameters()]
                #simple_adam_step(raw_model.transformer.h.parameters(), grads, exp_avg, exp_avg_sq, iteration_step, lr=1/(optimizer2_2.param_groups[0]['sigma_lr'] + optimizer2_2.param_groups[0]['rho_lr']), betas=(0.9, 0.999), eps=1e-8)
                #simple_adam_step(raw_model.transformer.h.parameters(), grads, exp_avg, exp_avg_sq, iteration_step, lr=optimizer2_2.param_groups[0]['lr'], betas=(0.9, 0.95), eps=1e-8)
                #simple_adam_step(raw_model.transformer.h.parameters(), grads, exp_avg, exp_avg_sq, iteration_step, lr=0.5*optimizer1.param_groups[0]['lr'], betas=(0.9, 0.95), eps=1e-8)


        '''for p in raw_model.lm_head.parameters():
        ...
        for p in model.parameters():
            if p.requires_grad and p.grad is not None:
                ...
                #dist.all_reduce(p.grad, op=dist.ReduceOp.SUM)
        '''grad = next(p.grad for p in raw_model.lm_head.parameters() if p.requires_grad)
        print(f"Rank {dist.get_rank()}, Grad Norm: {grad.norm()}")
        print('?????????????????????????????????')'''
        optimizer1.step()
        #optimizer2.step()
        schedulers_adam.step()
        #schedulers_adam2.step()
        #sched_sigma_rho_1.step()
        #sched_sigma_rho.step()
    ...
    if master_process:
        ...
        #np.savetxt(save_path_val, np.array(val_loss_record), fmt='%.6f')
        ...
        #np.savetxt(save_path_time, np.array([t.cpu().item() for t in train_time_record]), fmt='%.6f')
