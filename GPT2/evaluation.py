...
#model_save_path = "/workspace1/ow120/DDAM/gpt2-pytorch/gpt_finetuning/01-Recipes-Generation-GPT2/SPiAM_GPT_Small/AdamW/model_epoch_14/"
model_save_path = "/workspace1/ow120/DDAM/gpt2-pytorch/gpt_finetuning/01-Recipes-Generation-GPT2/SPiAM_GPT_Small/SPiAM/model_epoch_14/"
...
def infer(prompt, model):
    ...
    output = model.generate( input_ids.to(device), attention_mask=attention_mask.to(device), max_new_tokens=max_length, num_beams=5,  no_repeat_ngram_size=2, early_stopping=True)
...
##################### Validation after fine-tuning ##################### 
    ...
    print('#########################################################################')
    test_record.append(sum(scores_after)/len(scores_after))
######################################################################### 
print('The average semantic similarty is:', sum(test_record)/len(test_record))