...
sys.stdout = Logger("/workspace1/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/wgan-gp/wgan_2.txt")
...
parser.add_argument('--d_lr', type=float, default=0.0004, help='learning rate, default=0.0002')
parser.add_argument('--g_lr', type=float, default=0.0001, help='learning rate, default=0.0002')
...
opt.outf = 'SPiAM' + '-wgan_2' + '-betas{}-{}'.format(opt.beta1, opt.beta2) + '-eps{}-{}'.format(opt.eps, opt.eps_sqrt) \
...
rho_lr_generator = 1/opt.g_lr - sigma_lr_generator
rho_lr_discriminator = 1/opt.d_lr - sigma_lr_discriminator
...
if opt.optimizer == 'adam':
    optimizerD = optim.Adam(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
    optimizerG = optim.Adam(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
elif opt.optimizer == 'adamw':
    optimizerD = AdamW(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
    optimizerG = AdamW(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
elif opt.optimizer == 'fromage':
    optimizerD = Fromage(netD.parameters(), lr=opt.d_lr)
    optimizerG = Fromage(netG.parameters(), lr=opt.g_lr)
elif opt.optimizer == 'adabelief':
    optimizerD = AdaBelief(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
    optimizerG = AdaBelief(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps)
elif opt.optimizer == 'sgd':
    optimizerD = torch.optim.SGD(netD.parameters(), lr=opt.d_lr)
    optimizerG = torch.optim.SGD(netG.parameters(), lr=opt.g_lr)
elif opt.optimizer == 'rmsprop':
    optimizerD = torch.optim.RMSprop(netD.parameters(), lr=opt.d_lr)
    optimizerG = torch.optim.RMSprop(netG.parameters(), lr=opt.g_lr)
elif opt.optimizer == 'adabound':
    optimizerD = AdaBound(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps,
                          final_lr=opt.final_lr)
    optimizerG = AdaBound(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2), eps=opt.eps,
                          final_lr = opt.final_lr)
elif opt.optimizer == 'yogi':
    optimizerD = Yogi(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2),eps=opt.eps)
    optimizerG = Yogi(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2),eps=opt.eps)
elif opt.optimizer == 'radam':
    optimizerD = RAdam(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2),eps=opt.eps)
    optimizerG = RAdam(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2),eps=opt.eps)
elif opt.optimizer == 'msvag':
    optimizerD = MSVAG(netD.parameters(), lr=opt.d_lr, betas=(opt.beta1, opt.beta2))
    optimizerG = MSVAG(netG.parameters(), lr=opt.g_lr, betas=(opt.beta1, opt.beta2))
    ...
    for epoch in range(opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            for sb in range(opt.num_gpu):
                ...
                zero_grad(netD.parameters())
                real_cpu = data_sub.cuda()#Variable(imgs.type(Tensor))
                label = torch.full((real_cpu.size(0),), real_label).cuda()
                real_cpu=0.9*real_cpu+0.1*torch.randn((real_cpu.size())).cuda()
                output = netD(real_cpu).view(-1)
                errD_real = criterion(output, label)
                errD_real.backward()
                noise = torch.randn(real_cpu.size(0), nz, 1, 1)
                fake = netG(noise.cuda())
                label.fill_(fake_label)
                
                noise_add = torch.randn((fake.size()))
          
                fake=0.9*fake+0.1*noise_add.cuda()
                noise_input_record.append(noise)
                noise_add_record.append(noise_add)
            ...
            if i % opt.n_critic == 0:
                ...
                for sb in range(opt.num_gpu):
                    ...
                    noise = noise_input_record[sb]
                    noise_add = noise_add_record[sb]
                    fake = netG(noise.cuda())
                    fake=0.9*fake+0.1*noise_add.cuda()                    
