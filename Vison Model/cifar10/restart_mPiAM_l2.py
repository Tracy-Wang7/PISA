...
sys.stdout = Logger("/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/densenet_2.txt")
...
def get_parser():
    ...
    parser.add_argument('--restart_epoch', default=10, type=float, help='restart w_global epoch ')
...
def adjust_learning_rate(learning_rate, epoch, step_size=25, gamma=0.5, reset = False):
...
def main():
    ...
    W_global_previous = [param.clone() for param in W_n_0] 
    W_global_restart = [param.clone() for param in W_n_0]  
    ...
    W_global = generate_W_global(args.num_gpu, W_b_initial, P_b_initial, sigma_lr, alpha_b, args.l2_lambda)
    ...
    for epoch in range(start_epoch + 1, args.total_epoch):
        ...
        #if epoch %args.restart_epoch == 0 and epoch > args.restart_epoch:
        if epoch %args.restart_epoch == 0 and epoch > 0:
            with torch.no_grad():
                for w_previous, w,  w_restart in zip(W_global_previous, W_global, W_global_restart):
                        w_restart.copy_(w_previous/epoch + ((epoch-1)*w)/epoch)
                        global_restart_trigger = True
                        
        #if epoch % args.restart_epoch==0 and epoch >0:  ### when epoch  ==  restart_epoch, no restart
        if epoch % args.restart_epoch==0:
            with torch.no_grad():
                for w_previous, w in zip(W_global_previous, W_global):
                        w_previous.copy_(w)
                        
        #momentum type
        '''if epoch %args.restart_epoch == 0 and epoch > args.restart_epoch:
            with torch.no_grad():
                for w,  w_restart in zip(W_global, W_global_restart):
                        w_restart.copy_(w_restart/epoch + ((epoch-1)*w)/epoch)
                        global_restart_trigger = True
                        
        if epoch == args.restart_epoch:  ### when epoch  ==  restart_epoch, no restart
            with torch.no_grad():
                for w_restart, w in zip(W_global_restart, W_global):
                        w_restart.copy_(w)'''
        ...
        for iter_idx, (images, target) in enumerate(train_loader):
            ...
            ################ add more restart ################
            '''restart_count = iter_idx + epoch*len(train_loader)
            if restart_count %args.restart_epoch == 0 and restart_count > args.restart_epoch:
                with torch.no_grad():
                    for w_previous, w,  w_restart in zip(W_global_previous, W_global, W_global_restart):
                            w_restart.copy_(w_previous/restart_count + ((restart_count-1)*w)/restart_count)
                            global_restart_trigger = True
                            
            if restart_count %args.restart_epoch==0 and restart_count >0:  ### when epoch  ==  restart_epoch, no restart
                with torch.no_grad():
                    for w_previous, w in zip(W_global_previous, W_global):
                            w_previous.copy_(w)'''
            ...
            for sb in range(args.num_gpu):
                if global_restart_trigger:
                    print('Restart!')
                    with torch.no_grad():  # Disable gradient tracking
                        for param, w in zip(model.parameters(), W_global_restart):
                            param.copy_(w)
                ...
                with torch.no_grad():
                    ...
                    for i, (param_wn, param_pn, gradient, param_wg, accumulator, velocity) in enumerate(zip(W_n, P_n, gradients, model.parameters(), accumulators, velocities)):
        ...
        if test_acc > best_acc:
            ...
            #torch.save(model.state_dict(), "/workspace1/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/ResNet_best_model/model.pth")
            ...
            torch.save(model.state_dict(), "/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/DenseNet_best_model/model_2.pth")
        ...
        if not os.path.isdir('curve_restart'):
            os.mkdir('curve_restart')
        torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies},
                   os.path.join('curve_restart', ckpt_name))
    print('The best testing accuracy is:', best_acc)
