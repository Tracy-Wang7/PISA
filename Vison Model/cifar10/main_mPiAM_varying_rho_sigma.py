...
def adjust_learning_rate(learning_rate, epoch, step_size=25, gamma=0.5, reset = False):
    ...
    if epoch % step_size==0 and epoch>61: #for vggg
        learning_rate *= gamma
    if epoch % 25==0 and epoch>100: # for resnet
        learning_rate *= 0.5
...
def main():
    ...
    for epoch in range(start_epoch + 1, args.total_epoch):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(), "/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/VGG_best_model/model.pth")
        ...
        if not os.path.isdir('curve_new'):
            os.mkdir('curve_new')
        torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies},
                   os.path.join('curve_new', ckpt_name))
