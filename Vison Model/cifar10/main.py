...
sys.stdout = Logger("/workspace1/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/vgg_lamb_2.txt")
...
def get_parser():
    ...
    parser.add_argument('--optim', default='sgd', type=str, help='optimizer',
                        choices=['sgd', 'adam', 'adamw', 'adabelief', 'yogi', 'msvag', 'radam', 'fromage', 'adabound', 'lamb'
                                 ])
...
def get_ckpt_name(model='resnet', optimizer='sgd', lr=0.1, final_lr=0.1, momentum=0.9,
    ...
    name = {
        ...
        'lamb': 'lr{}-wdecay{}-eps{}-run{}'.format(lr,weight_decay, eps, run),
...
def create_optimizer(args, model_params):
    ...
    if args.optim == 'sgd':
        ...
        return opt.Lamb(model_params, lr=args.lr, weight_decay=args.weight_decay, eps=args.eps)
    else:
...
def main():
    ...
    for epoch in range(start_epoch + 1, args.total_epoch):
        start = time.time()
        adjust_learning_rate(optimizer, epoch, step_size=args.decay_epoch, gamma=args.lr_gamma, reset = args.reset) # here train_acc is training loss
        ...
        torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies},
                   os.path.join('curve_resnet', ckpt_name))
