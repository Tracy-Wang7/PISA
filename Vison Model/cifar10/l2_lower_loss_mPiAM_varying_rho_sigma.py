...
sys.stdout = Logger("/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/tmux_vgg_1.txt")
...
def average_parameters_with_normalized(num_train_env, list_vars, list_alpha, list_nomalized):
    sum_vars = [torch.zeros_like(var) for var in list_vars[0]]
    for i in range(num_train_env):
        W_n = list_vars[i]
        alpha = list_alpha[i]
        normalized_factor = list_nomalized[i]
        sum_vars = [sum_ + alpha*update/normalized_factor for sum_, update in zip(sum_vars, W_n)]
    return sum_vars

def generate_W_global_normalized(num_batches, W_n_list, P_n_list, tau_lr, alpha, list_nomalized, l2_lambda):
    W_n_avg = average_parameters(num_batches, W_n_list, alpha)
    P_n_avg = average_parameters_with_normalized(num_batches, P_n_list, alpha, list_nomalized)
...
def adjust_learning_rate(learning_rate, epoch, step_size=25, gamma=0.5, reset = False):
    ...
    if epoch % 5==0 and epoch>21 and epoch <61:
        learning_rate *= gamma
    if epoch % step_size==0 and epoch>0 and epoch <21:
        learning_rate *= gamma
    #if epoch % step_size==0 and epoch>61: #for testing acc reach 91.08, as default
    if epoch % step_size==0 and epoch>61 and epoch < 150:
        learning_rate *= gamma
    if epoch % 25==0 and epoch>100: #for testing acc reach 91.08, as default
    #if epoch % 25==0 and epoch>100 and epoch<150:
        learning_rate *= 0.35
        
        
    '''if epoch % 25==0 and epoch>100 and epoch<150: # first attempt
        learning_rate *= 0.5
...
def main():
    ...
    seed = args.seed  # e.g. from an argparse argument
    random.seed(seed)
    np.random.seed(seed)
    ...
    for epoch in range(start_epoch + 1, args.total_epoch):
        ...
        for iter_idx, (images, target) in enumerate(train_loader):
            ...
            for sb in range(args.num_gpu):
                ...
                if epoch == 0 and iter_idx ==0:
                    factor = torch.sqrt(sum(torch.sum(g ** 2) for g in gradients if g is not None))
                ...
                with torch.no_grad():
                    ...
                    for i, (param_wn, param_pn, gradient, param_wg, accumulator, velocity) in enumerate(zip(W_n, P_n, gradients, W_global, accumulators, velocities)):
                        ...
                        delta = param_wg -  (gradient+ param_pn)/(sigma_lr_current* normalized_factor[sb]+ rho_lr_current * normalized_factor[sb] * (torch.sqrt(corrected_accumulator) + args.eps))
            ...
            with torch.no_grad():
                ...
                W_global = generate_W_global_normalized(args.num_gpu, W_b_initial, P_b_initial, sigma_lr_current, alpha_b, normalized_factor, args.l2_lambda)
                for param, w in zip(model.parameters(), W_global):
                    param.copy_(w)
        ...
        if test_acc > best_acc:
            ...
            #torch.save(model.state_dict(), "/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/classification_cifar10/VGG_best_model/model_2.pth")
