...
sys.stdout = Logger("/workspace0/ow120/DDAM/Graph_NN_case/cifar_10/Adabelief-Optimizer/PyTorch_Experiments/imagenet/imagenet_1.txt")
...
parser.add_argument('--data', metavar='DIR',default='/workspace0/ow120/DDAM/Imagenet/dataset/imagenet/',#'/data1/ILSVRC2012',#
...
def generate_W_global_l2(num_batches, W_n_list, P_n_list, tau_lr, alpha, weight_decay):
    W_n_avg = average_parameters(num_batches, W_n_list, alpha)
    P_n_avg = average_parameters(num_batches, P_n_list, alpha)
    for i in range(len(W_n_avg)):
        W_n_avg[i] = (tau_lr*W_n_avg[i] + P_n_avg[i])/(tau_lr + weight_decay)
        W_n_avg[i].detach()
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.optimizer == 'sgd' and (not args.centralize):
        ...
        optimizer = torch.optim.RMSprop(model.parameters(), args.lr, alpha=args.beta_rmsprop)
        
    elif args.optimizer == 'adam':
        optimizer = torch.optim.Adam(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2))
    ...
    W_global = generate_W_global_l2(args.num_gpu, W_b_initial, P_b_initial, sigma_lr, alpha_b, args.l2_lambda)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for iter_idx, (images, target) in enumerate(train_loader):
            ...
            output = model(images)
            acc1_sub, acc5_sub = accuracy(output, target, topk=(1, 5))
            ...
            for sb in range(args.num_gpu):
                ...
                with torch.no_grad():
                    ...
                    for i, (param_wn, param_pn, gradient, param_wg, accumulator, velocity) in enumerate(zip(W_n, P_n, gradients, W_global, accumulators, velocities)):
                        ...
                        param_wg_l2 = param_wg* (1 - args.weight_decay/(sigma_lr_current+ rho_lr_current))
                        bias_correction2 = 1 - args.beta_rmsprop** updated_iteration                        
                        corrected_accumulator = accumulator / (bias_correction2)
                        #param_wg_l2 = param_wg* (1 - args.weight_decay/(sigma_lr_current+ rho_lr_current*(torch.sqrt(corrected_accumulator) + args.eps)))
                        #delta = param_wg -  (gradient + param_pn)/(sigma_lr_current + rho_lr_current*(torch.sqrt(corrected_accumulator) + args.eps))
                        delta = param_wg_l2 -  (gradient+ param_pn)/(sigma_lr_current + rho_lr_current*(torch.sqrt(corrected_accumulator) + args.eps))
            ...
            with torch.no_grad():
                W_global = generate_W_global_l2(args.num_gpu, W_b_initial, P_b_initial, sigma_lr_current, alpha_b, args.l2_lambda)
